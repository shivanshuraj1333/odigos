...
func updateInstrumentationConfigSpec(ctx context.Context, c client.Client, pw k8sconsts.PodWorkload, ic *odigosv1.InstrumentationConfig, distroProvider *distros.Provider, effectiveConfig *common.OdigosConfiguration) (*agentInjectedStatusCondition, error) {
	...
	for containerName, containerRuntimeDetails := range runtimeDetailsByContainer {
		...
		// AgentAppliesUrlTemplatization must match the webhook's injection condition exactly (pods_webhook.go):
		// the webhook injects ODIGOS_AGENT_URL_TEMPLATIZATION only when AgentEnabled, non-empty rules, distro supports it, and ConfigAsEnvVars.
		agentAppliesUrlTemplatization := false
		if currentContainerConfig.AgentEnabled && workloadUrlTemplatization != nil && len(workloadUrlTemplatization.TemplatizationRules) > 0 && containerRuntimeDetails != nil {
			if d, errCfg := resolveContainerDistro(containerName, containerOverride, containerRuntimeDetails.Language, distroPerLanguage, distroProvider.Getter); errCfg == nil {
				agentAppliesUrlTemplatization = d.Traces != nil && d.Traces.UrlTemplatization != nil && d.Traces.UrlTemplatization.Supported && d.ConfigAsEnvVars
			}
		}
		// calculate the relevant collector configurations for the container.
		currentContainerCollectorConfig := calculateContainerCollectorConfig(containerName, effectiveConfig, containerRuntimeDetails, distroPerLanguage, distroProvider.Getter, containerOverride, samplingRules, workloadUrlTemplatization, agentAppliesUrlTemplatization, pw)
...
func calculateContainerCollectorConfig(containerName string,
	...
	agentAppliesUrlTemplatization bool,
	...
	// agentAppliesUrlTemplatization is computed in the caller to match the webhook's injection condition exactly
	// (AgentEnabled + non-empty rules + distro supports + ConfigAsEnvVars). When true, the gateway skips this container's spans.
	return &commonapi.ContainerCollectorConfig{
		...
		AgentAppliesUrlTemplatization: agentAppliesUrlTemplatization,
	}
}
