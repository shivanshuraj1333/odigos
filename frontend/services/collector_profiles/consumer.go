	...
	return xconsumer.NewProfiles(func(ctx context.Context, pd pprofile.Profiles) error {
		...
		for i := 0; i < n; i++ {
			...
			if !ok {
				...
				// Log attribute values when debug enabled so we can compare container.id with k8s (e.g. containerd:// prefix).
				if os.Getenv("PROFILING_DEBUG") == "true" {
					attrs := rp.Resource().Attributes()
					if cid, has := attrs.Get("container.id"); has {
						log.Printf("[profiling] receiver: dropped resource attrs container.id=%q k8s.node.name=%q (k8sattributes in node collector may not be enriching; expect k8s.namespace.name, k8s.deployment.name)",
							cid.Str(), getAttrStr(attrs, "k8s.node.name"))
					}
				}
				continue
			}
...
func getAttrStr(attrs pcommon.Map, key string) string {
