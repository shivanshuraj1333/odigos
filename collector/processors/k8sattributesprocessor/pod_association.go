...
func extractPodID(ctx context.Context, attrs pcommon.Map, associations []kube.Association) kube.PodIdentifier {
	candidates := extractPodIDCandidates(ctx, attrs, associations)
	if len(candidates) == 0 {
...
func extractPodIDCandidates(ctx context.Context, attrs pcommon.Map, associations []kube.Association) []kube.PodIdentifier {
	// If pod association is not set
	if len(associations) == 0 {
		pid := extractPodIDNoAssociations(ctx, attrs)
		if pid.IsNotEmpty() {
			return []kube.PodIdentifier{pid}
		}
		return nil
	}

	var candidates []kube.PodIdentifier
		...
		for i, source := range asso.Sources {
			...
			case kube.ResourceSource:
				...
				if source.Name == string(conventions.ContainerIDKey) {
					attributeValue = normalizeContainerID(attributeValue)
				}
