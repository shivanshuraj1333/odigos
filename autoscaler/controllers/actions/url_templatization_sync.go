...
func buildUrlTemplatizationProcessor(namespace string, spanMetricsEnabled bool) (*odigosv1.Processor, error) {
	...
	// One shared Processor CR: gateway-only or node-only based on span metrics (node CollectorsGroup).
...
// so the CR matches for url templatization matches the desired state
func SyncUrlTemplatizationProcessor(ctx context.Context, c client.Client, mode URLTemplatizationSyncMode) error {
	...
	// URLTemplatizationSyncCreateIfMissing (Action reconcile only): if the shared Processor
	// already exists, skip the rest of this function. That avoids extra CollectorsGroup reads
	// and repeated server-side apply on hot Action reconciles.
	//
	// Spec drift while the CR exists (e.g. CollectorRoles when span metrics toggles) is
	// reconciled by controllers that call URLTemplatizationSyncApplyFull instead.
	//
	// If Get returns NotFound, fall through to build + Patch below. Other Get errors also fall
