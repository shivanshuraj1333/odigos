...
func (kp *kubernetesprocessor) processResource(ctx context.Context, resource pcommon.Resource, signalType string) {
	podIdentifierCandidates := extractPodIDCandidates(ctx, resource.Attributes(), kp.podAssociations)
	var podIdentifierValue kube.PodIdentifier
	if len(podIdentifierCandidates) > 0 {
		podIdentifierValue = podIdentifierCandidates[0]
	}
	...
	if podIdentifierValue.IsNotEmpty() {
		for _, candidate := range podIdentifierCandidates {
			if pod, podFound = kp.kc.GetPod(candidate); podFound {
				podIdentifierValue = candidate
				podIdentifierStr = buildPodIdentifierString(candidate)
				break
			}
		}
		if podFound {
	...
	if !podFound {
		kp.logPodAssociationMiss(resource.Attributes(), podIdentifierCandidates, signalType)
	}
...
func (kp *kubernetesprocessor) logPodAssociationMiss(attrs pcommon.Map, candidates []kube.PodIdentifier, signalType string) {
	...
	case len(candidates) == 0:
		missType = "no_candidate"
	case containerID == "" && podIP == "":
		missType = "no_container_id_or_pod_ip"
	case containerID == "":
		missType = "no_container_id"
	case podIP == "":
		missType = "no_pod_ip"
	default:
		missType = "candidate_lookup_miss"
	}

	kp.logger.Debug(
		"k8sattributes association miss",
		zap.String("miss_type", missType),
		zap.String("signal", signalType),
		zap.String("container_id", containerID),
		zap.String("k8s_pod_ip", podIP),
