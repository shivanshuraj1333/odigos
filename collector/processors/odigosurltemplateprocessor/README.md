...
### Span Name Normalization When Target Attribute Is Already Set

When an agent (e.g. a Java or Node.js OpenTelemetry SDK instrumentation library) already
sets the target attribute (`http.route` for server spans, `url.template` for client spans),
this processor no longer skips those spans entirely. Instead, it normalizes the span name to
`{method} {target}` using the existing attribute value.

For example, a span with name `"GET /user/1234"` (a high-cardinality name that the
instrumentation wrote directly) and `http.route = "/user/{id}"` (set by the agent) will have
its name corrected to `"GET /user/{id}"`. The attribute itself is left unchanged.

This ensures that span names are low-cardinality and compliant with HTTP semantic conventions
even when the agent sets the route template directly, which was previously not the case.
