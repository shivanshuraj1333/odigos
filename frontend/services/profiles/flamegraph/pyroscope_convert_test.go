...
func TestTryPyroscopeOTLP_AcceptsBinaryFromPdataProtoMarshaler(t *testing.T) {
	...
	_, _, reason := tryPyroscopeOTLP(b)
	require.False(t, strings.HasPrefix(reason, "protojson_unmarshal:"),
		"binary pdata chunks must not be parsed only as JSON; got: %s", reason)
}
