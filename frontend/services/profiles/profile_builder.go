...
func BuildPyroscopeProfileFromChunks(chunks [][]byte) flamegraph.FlamebearerProfile {
	...
	up, tree, err := flamegraph.BuildFlamebearerViaPyroscopeSymdb(context.Background(), chunks, maxNodes)
	symbolsHint := ""
	if err != nil {
	...
	if up != nil && numTicks > 0 && allNamesArePlaceholders(up.Flamebearer.Names) {
		if symbolsHint == "" {
