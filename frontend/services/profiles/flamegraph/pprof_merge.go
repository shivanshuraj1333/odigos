	...
	for _, rp := range req.ResourceProfiles {
		...
		for _, sp := range rp.ScopeProfiles {
			...
			for _, p := range sp.Profiles {
				...
				// Do not inject service.name into sample attributes. Pyroscope's ConvertOtelToGoogle
				// reads it only to key its returned map[string]convertedProfile, and we discard that
				// keying below (we merge all profiles into one flamegraph). ConvertOtelToGoogle also
				// strips service.name from the Google profile's sample labels (grafana/pyroscope
				// pkg/ingester/otlp/convert.go convertSampleAttributesToLabelsBack), so any mutation
				// we did on OTLP samples would not survive the conversion anyway. If a future feature
				// needs service.name for flamegraph metadata, read it from rp.Resource.Attributes
				// directly — that mirrors grafana/pyroscope's own ingest_handler.export().
