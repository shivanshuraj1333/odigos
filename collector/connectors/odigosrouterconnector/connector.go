...
func (r *routerConnector) ConsumeProfiles(ctx context.Context, pd pprofile.Profiles) error {
	...
	// Copy the shared ProfilesDictionary so routed batches have valid index references.
	pd.Dictionary().CopyTo(defaultProfiles.Dictionary())
	var errs error
	...
	for i := 0; i < rProfiles.Len(); i++ {
		...
		for _, pipeline := range pipelines {
			...
			if !ok {
				...
				// Copy the shared ProfilesDictionary so routed batches have valid index references.
				pd.Dictionary().CopyTo(batch.Dictionary())
			}
