...
func TestAddProfilingGatewayPipeline_Enabled(t *testing.T) {
	...
	err := addProfilingGatewayPipeline(&c, "odigos-system", &common.ProfilingConfiguration{Enabled: &on}, gw, nil)
...
func TestAddProfilingGatewayPipeline_GatewayUiOtlpEndpointOverride(t *testing.T) {
	on := true
	var c config.Config
	gw := &odigosv1.CollectorsGroup{
		Spec: odigosv1.CollectorsGroupSpec{
	...
	prof := &common.ProfilingConfiguration{
		Enabled:               &on,
		GatewayUiOtlpEndpoint: "10.0.1.50:4317",
	}
