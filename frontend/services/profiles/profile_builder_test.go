...
func TestBuildProfile_JSONShape(t *testing.T) {
	prof := buildPyroscopeProfileFromChunks(context.Background(), nil)
...
func TestBuildProfile_EndToEnd_OTLPMergeAcrossChunks(t *testing.T) {
	a := minimalOTLPProfilesExportWire(t, 5)
	b := minimalOTLPProfilesExportWire(t, 7)
	prof := buildPyroscopeProfileFromChunks(context.Background(), [][]byte{a, b})
...
func TestBuildProfile_InvalidWireDoesNotPanic(t *testing.T) {
	prof := buildPyroscopeProfileFromChunks(context.Background(), [][]byte{[]byte("not-valid-protobuf"), nil, {}})
	_, err := json.Marshal(prof)
	require.NoError(t, err)
}

// TestE2E_SymdbPath_UIMatchesRawFlamebearer_MinimalChunks verifies buildPyroscopeProfileFromChunks (UI path)
...
func TestE2E_SymdbPath_UIMatchesRawFlamebearer_MinimalChunks(t *testing.T) {
	...
	prof := buildPyroscopeProfileFromChunks(context.Background(), chunks)
