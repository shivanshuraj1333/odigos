...
interface ClearProfilingBufferResult {
  status: string;
  sourceKey: string;
  activeSlots: number;
}
...
interface UseProfiling {
  ...
  clearProfilingBuffer: (source: SourceIdentifier) => Promise<ClearProfilingBufferResult | undefined>;
...
export const useProfiling = (): UseProfiling => {
  ...
  const [mutateClearBuffer] = useMutation<{ clearSourceProfilingBuffer: ClearProfilingBufferResult }, SourceIdentifier>(CLEAR_SOURCE_PROFILING_BUFFER, {
    onError: (e) => logProfilingError('clearSourceProfilingBuffer failed', e),
  });
  ...
  // Drops buffered OTLP profile chunks but keeps the profiling slot active (flame graph reset).
  const clearProfilingBuffer: UseProfiling['clearProfilingBuffer'] = useCallback(
    async (source) => {
      const { data, errors } = await mutateClearBuffer({ variables: source });
      if (errors?.length) logProfilingError('clearSourceProfilingBuffer returned GraphQL errors', errors);
      return data?.clearSourceProfilingBuffer;
    },
    [mutateClearBuffer],
  );
