...
const (
	// defaultMaxNodes matches Grafana Pyroscope querier SelectMergeStacktraces default (maxNodesDefault).
...
func TreeToFlamebearer(t *Tree, maxNodes int64) Flamebearer {
	...
	for len(stack) > 0 {
		...
		// Match github.com/grafana/pyroscope/pkg/model.NewFlameGraph: advance by n.self before children so
		// sum(children widths)+self equals parent.total. Otherwise self>0 leaves empty space (holes) under the bar.
		xStart := cur.xOffset + n.self
	...
	// Delta-encode x offsets (first of each 4-tuple) to match Pyroscope wire format.
	// flattenFlameRects on the frontend expects delta-encoded positions.
