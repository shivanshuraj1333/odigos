  ...
  return (
    <Panel>
      <FlexColumn style={{ gap: 8 }}>
        ...
        <Help>
          Loads aggregated CPU profile data for a workload (same JSON as <code>GET /api/sources/…/profiling</code>). Use the real Kubernetes{' '}
      ...
      <Row>
        ...
        <Button variant="primary" disabled={!canSubmit || loading} onClick={onEnableAndLoad}>
          {loading ? 'Loading…' : 'Enable & load profile'}
        </Button>
        <Button variant="secondary" disabled={!canSubmit || loading} onClick={onRefresh}>
          Refresh
        </Button>
