...
// BoundedBuffer is a byte-budgeted FIFO of raw profile chunks. Whole oldest chunks
// are evicted when the total exceeds maxBytes; evicted chunks are zeroed and the
// backing slice is compacted so it does not grow without bound under churn.
...
// AddAt stores a chunk with an explicit capture time. It returns false only when a
// single chunk exceeds the whole budget and can never be retained.
